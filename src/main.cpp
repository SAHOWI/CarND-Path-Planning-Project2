...
int main() {
  ...
  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,
	...
	// Some code similar to what has been shown in the QA
	...
	int gap = 30; // 30m -> magic value considered safe (in this situation) between our car and the next car ahead
	...
	// Change lanes if it is safe to do so (nobody to the side)
	double acc = 0.224; // Safe value as shown in the QA 
	double max_speed = 49.5;
	if (too_close) {
		// A car is ahead
		// Decide to change lanes or slow down if not possible
		if (!car_right && lane < 2) {
			// If there is no other car in the right lane -> change right
			lane++;
		} else if (!car_left && lane > 0) {
			// If there is no other car in the left lane -> change left
			lane--;
		} else {
	...
	// using 30, 60 and 90 m waypoints in the future
